...
export function setAuthority(value) {
  const { userId, loginTime, userName } = value;
...
export function checkPassword(password) {
  if (password.length < 6) {
    message.error('密码应不少于6个字符');
    return false;
  } else if (!/^(?![0-9]+$)(?![a-zA-Z]+$)[a-zA-Z0-9]{6,20}/.test(password)) {
    message.error('密码必须由数字和字母组成');
    return false;
  }
  return true;
}

export function Status2Name(aObj, status) {
  const obj = aObj.find(item => item.status === status);
  if (obj && obj.name) {
    return obj.name;
  }
  return status;
